...
export function usePoolManager(
  ...
  const loadPools = async () => {
    ...
    try {
      const { result: poolsData } = await apiCall(
        `${SERVER_URL}/pools?creatorAddress=${encodeURIComponent(walletAddress)}`,
        {
          method: "GET",
  ...
  const handleCreatePool = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showError("Creation Failed", `Error creating pool: ${error instanceof Error ? error.message : String(error)}`)
    }
  }
  ...
  const handleDownloadWallet = async () => {
    ...
    try {
      ...
      showError(
        "Download Failed",
        `Error downloading wallet: ${error instanceof Error ? error.message : String(error)}`,
      )
    }
  }
  ...
  const handleSponsorCredits = async () => {
    ...
    try {
      ...
      if (successfulShares > 0) {
        ...
        showError(
          "Sponsor Failed",
          errors.length > 0 ? `No credits sponsored. Errors: ${errors.join("; ")}` : "No credits sponsored.",
        )
      }
